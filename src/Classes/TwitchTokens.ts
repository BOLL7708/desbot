...
export default class TwitchTokens {
     ...
     */
    async refreshToken() {
        let channelTokenData = await DataBaseHelper.loadSetting(new SettingTwitchTokens(), 'Channel')
        ...
        let chatbotTokenData = await DataBaseHelper.loadSetting(new SettingTwitchTokens(), 'Chatbot')
    ...
    private async refresh(data: SettingTwitchTokens, key: string) {
        const clientData = await DataBaseHelper.loadSetting(new SettingTwitchClient(), 'Main')
        ...
        if (!json.error && !(json.status >= 300)) {
            ...
            const success = await DataBaseHelper.saveSetting(data, key)
